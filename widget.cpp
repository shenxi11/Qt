    ...
    , ui(new Ui::Widget)
{
    ...
    connect(ui->button,&QPushButton::clicked,[=](){
        ...
        id1=startTimer(5000);
        //id2=startTimer(30,Qt::PreciseTimer);
...
void Widget::timerEvent(QTimerEvent*t){
    ...
    if(id==id1){
        ...
        for(vector<Bullet*>::iterator it=bullets.begin();it!=bullets.end();++it){
            if(!(*it)->getPult()){
                ...
                if(mind==114514){
                    ...
                    (*it)->dx+=5;
                    float yu=180.0*(float)((*it)->getLabel()->x()-(*it)->getStartPos().x())/(float)(q->getLabel()->x()-(*it)->getStartPos().x());
                    (*it)->getLabel()->move((*it)->getStartPos().x()+(*it)->dx,(*it)->getStartPos().y()-100*qfsind(yu));


                }
                else if((*it)->getLabel()->x()>=q->getLabel()->x()&&(*it)->getLabel()->x()<=q->getLabel()->x()+q->getLabel()->width()){
                    (*it)->setBulletAttack(true);
                    q->be_attack(50);
                    //qDebug()<<(*it)->getIndex();
        ...
        zombiesDie();
        renewZombieIndex();
        //int msecond=time.elapsed();
        //qDebug()<<"hua"<<msecond<<"毫秒";
                ...
                if(!map[x][y]&&plants[qq->getNum()]->getSunCost()<=this->sunNum&&plants[qq->getNum()]->getIfCD()){
                    ...
                    if(a->getIsAtk()==1){
                        ...
                        connect(a,&Plant::Shot,[=](){
                            ...
                            Bullet*b=new Bullet(a->getBullet()->getV(),a->getBullet()->getLength(),a->getBullet()->getPult());
                            b->setLabel(h);
                            b->setATK(a->getATK());
                            bullets.push_back(b);
